[![pipeline status](https://gitlab.com/aruw/controls/uta-robotics/badges/develop/pipeline.svg)](https://gitlab.com/aruw/controls/uta-robotics/-/commits/develop)
...
```
cd my-amazing-project/uta-robotics-project
pipenv shell
# Build for hardware
scons build
# Run automated tests
...
1. Make sure you have VSCode opened in the folder `uta-robotics` (**not
   `uta-robotics-project`**)
...
1. Open the folder `aruw-uta-robotics-project` in VSCode. Hit the debug tab on the left side or type
...
`/uta-robotics-project/robot-type/robot_type.hpp` and change the macro defined in this file.
...
The below commands require that your working directory is `/uta-robotics-project` (where the
